        ...
        <div class="col-span-4">
            <x-filament::section>
                ...
                <x-filament::section class="bg-gray-100">
                    <div class="grid grid-cols-5 gap-4 print:grid-cols-5">
                        <div>
                        ...
                        <div>
                            ...
                            <p class="mt-1">{{ $record->effective_date?->format('d/m/Y') }}</p>
                        </div>

                        <div>
                            <p class="text-sm font-medium text-gray-500">Fecha Terminación</p>
                            <p class="mt-1">{{ $record->end_date?->format('d/m/Y') }}</p>
                        </div>
                        ...
                        <div class="col-start-5">
                            ...
                            <p class="mt-1">$ {{ $record->policy_prime }}</p>
                        </div>
                        ...
                        <div class="w-full">
                            <table class="w-full table-fixed divide-y divide-gray-200">
                                ...
                                @foreach($applicants as $applicant)
                                    <tr>
                                        {{-- <td class="px-6 py-4 text-sm font-medium text-gray-900">{{ $applicant->relationship === 'Aplicante Principal' ? 'Aplicante Principal' : FamilyRelationship::from($applicant->relationship)->getLabel() }}</td> --}}
                                        <td class="px-6 py-4 text-sm text-gray-900">{{ $applicant->age ?? 'N/A' }}</td>
                                        <td class="px-6 py-4 text-sm text-gray-900">{{ $applicant->gender }}</td>
                                    ...
                                    <tr>
                                        @if(!$applicant->is_self_employed)
                                            <td class="px-6 py-4 text-sm font-medium text-gray-900">
                                                @if($applicant->relationship === 'Aplicante Principal')
                                                    Aplicante Principal
                                                @else
                                                    {{ FamilyRelationship::from($applicant->relationship)->getLabel() }}
                                                @endif
                                            </td>
                                        ...
                                        @else
                                            @php
                                                $relation = $applicant->relationship === 'self' ? 'Aplicante Principal' : FamilyRelationship::from($applicant->relationship)->getLabel();
                                            @endphp
